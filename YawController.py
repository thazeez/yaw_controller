...
#run this way: python3 YawController.py --odom-topic /fmu/out/vehicle_odometry --yaw-step-deg 90 --kp-yaw 1.5 --yaw-rate-max 1.0 --tol-deg 2.0  --warmup 1.0 --hz 50 --logfile /home/root/yaw_log.csv

"""
YawController.py

Yaw-only controller (NO linear motion):
- Subscribes to PX4 odometry: /fmu/out/vehicle_odometry (quaternion + yaw rate)
...
def yaw_from_quat_body_to_ned(w, x, y, z):
    ...
    """
    # Rotation matrix elements (same as your PDController)
    r11 = 1 - 2 * (y * y + z * z)
    r21 = 2 * (x * y + z * w)
    # yaw = atan2(r21, r11)
...
class YawController(Node):
    def __init__(self, args):
        ...
        self.yaw_rate_meas = 0.0  # rad/s from PX4 (approx)
    ...
    def odom_cb(self, msg: VehicleOdometry):
        ...
        # VehicleOdometry.angular_velocity is body rates [p,q,r] in rad/s (FRD body)
        # Some PX4 builds fill it; if not, it may be zeros. We log it if present.
        try:
...
def main():
    ...
    p.add_argument("--logfile", default="/home/root/yaw_log.csv",
