#!/usr/bin/env python3
# how to run:
# python3 offboard_vel_relay_body3_fullquat_withyaw_owl.py --cmd-timeout 0.3 --offboard-prestream 0.8 --yawrate-max 1.0
...
FALCON VERSION:
- Uses Falcon-specific PX4 topics by default
...
Notes:
- PX4 VehicleOdometry.q is WXYZ in your system: q = [w,x,y,z]
...
class OffboardVelRelayBodyFullQuat(Node):
    def __init__(
        self,
        cmd_topic="/owl/cmd_vel",
        odom_topic="/owl/fmu/out/vehicle_odometry",
        offboard_mode_topic="/owl/fmu/in/offboard_control_mode",
        traj_sp_topic="/owl/fmu/in/trajectory_setpoint",
        vehicle_cmd_topic="/owl/fmu/in/vehicle_command",
        hz=50.0,
        ...
        self.yawrate_max = float(abs(yawrate_max))
        ...
        self.pub_cm = self.create_publisher(OffboardControlMode, offboard_mode_topic, qos_px4())
        self.pub_sp = self.create_publisher(TrajectorySetpoint, traj_sp_topic, qos_px4())
        self.pub_cmd = self.create_publisher(VehicleCommand, vehicle_cmd_topic, qos_px4())

        self.create_subscription(VehicleOdometry, odom_topic, self._odom_cb, qos_px4())
        ...
        if not log_path:
            ...
            log_path = f"/home/root/logs/offboard_body_relay_fullquat_withyaw_owl_{ts}_{run_id}.csv"
        ...
        self.get_logger().info(f"Listening cmd topic: {cmd_topic}")
        self.get_logger().info(f"Listening odom topic: {odom_topic}")
        self.get_logger().info(f"Publishing offboard mode topic: {offboard_mode_topic}")
        self.get_logger().info(f"Publishing trajectory setpoint topic: {traj_sp_topic}")
        self.get_logger().info(f"Publishing vehicle command topic: {vehicle_cmd_topic}")
        self.get_logger().info(f"BODY cmd convention: x=fwd, y=right, z=down, yawrate=angular.z")
...
def main():
    ...
    parser.add_argument("--cmd-topic", type=str, default="/owl/cmd_vel")
    parser.add_argument("--odom-topic", type=str, default="/owl/fmu/out/vehicle_odometry")
    parser.add_argument("--offboard-mode-topic", type=str, default="/owl/fmu/in/offboard_control_mode")
    parser.add_argument("--traj-sp-topic", type=str, default="/owl/fmu/in/trajectory_setpoint")
    parser.add_argument("--vehicle-cmd-topic", type=str, default="/owl/fmu/in/vehicle_command")
    ...
    parser.add_argument("--yawrate-max", type=float, default=1.0)
    ...
    node = OffboardVelRelayBodyFullQuat(
        ...
        offboard_mode_topic=args.offboard_mode_topic,
        traj_sp_topic=args.traj_sp_topic,
        vehicle_cmd_topic=args.vehicle_cmd_topic,
        hz=args.hz,
